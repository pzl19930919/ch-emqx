...
do_query(
    ...
    case apply_template(QueryTuple, Templates, ChannelState) of
...
%% single insert
apply_template(
    {?ACTION_SEND_MESSAGE = _Key, _Msg} = Query, Templates, ChannelState
) ->
    %% TODO: fix emqx_placeholder:proc_tmpl/2
    %% it won't add single quotes for string
    apply_template([Query], Templates, ChannelState);
%% batch inserts
apply_template(
    ...
    case maps:get(Key, BatchInsertsTks, undefined) of
        ...
        #{?BATCH_INSERT_PART := BatchInserts, ?BATCH_PARAMS_TOKENS := BatchParamsTks} ->
            SQL = proc_batch_sql(BatchReqs, BatchInserts, BatchParamsTks, ChannelState),
            {Key, SQL}
    end;
...
proc_batch_sql(BatchReqs, BatchInserts, Tokens, ChannelState) ->
    Values = erlang:iolist_to_binary(
        lists:join($,, [
            proc_msg(Tokens, Msg, ChannelState)
         || {_, Msg} <- BatchReqs
        ])
    ),
